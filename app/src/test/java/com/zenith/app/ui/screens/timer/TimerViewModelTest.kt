...
class TimerViewModelTest {
    ...
    fun setup() {
        ...
        // Default mocks
        coEvery { taskRepository.getTaskById(1L) } returns mockTask
        coEvery { settingsRepository.getPomodoroSettings() } returns defaultSettings
        coEvery { settingsRepository.getAllowedApps() } returns listOf("com.example.allowed1")
        ...
        every { context.packageName } returns "com.zenith.app"
    ...
    fun `initial state loads allowed apps from settings`() = runTest {
        // Arrange
        val savedAllowedApps = listOf("com.example.allowed1", "com.example.allowed2")
        coEvery { settingsRepository.getAllowedApps() } returns savedAllowedApps
    ...
    fun `uses task work duration when available`() = runTest {
        ...
        coEvery { taskRepository.getTaskById(1L) } returns taskWithDuration
    ...
    fun `uses default settings when task has no work duration`() = runTest {
        ...
        coEvery { taskRepository.getTaskById(1L) } returns taskWithoutDuration
    ...
    fun `loads pomodoro settings correctly`() = runTest {
        ...
        coEvery { settingsRepository.getPomodoroSettings() } returns customSettings
    ...
    fun `getAvailableBgmTracks returns tracks from BgmManager`() = runTest {
        // Arrange
        val mockTracks = listOf(mockk<com.zenith.app.domain.model.BgmTrack>())
    ...
    fun `handles missing task gracefully`() = runTest {
        // Arrange
        coEvery { taskRepository.getTaskById(1L) } returns null
