...
class BackupViewModel @Inject constructor(
    ...
    fun onEvent(event: BackupEvent) {
        when (event) {
            ...
            is BackupEvent.ResetState -> resetState()
            is BackupEvent.HandleGoogleSignInResult -> handleGoogleSignInResult(event.data)
            is BackupEvent.SignOutGoogle -> signOutGoogle()
            is BackupEvent.UploadToCloud -> uploadToCloud()
            is BackupEvent.DownloadFromCloud -> downloadFromCloud()
            is BackupEvent.ResetCloudState -> resetCloudState()
            is BackupEvent.StartTrial -> startTrial()
        }
    }
