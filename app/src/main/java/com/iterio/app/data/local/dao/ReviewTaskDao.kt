...
/**
 * 日付ごとの復習タスク数を格納するデータクラス
 */
data class DateReviewTaskCount(
    val date: LocalDate,
    val count: Int
)
...
interface ReviewTaskDao {
    ...
    // ========== WITH DETAILS (JOIN) QUERIES ==========

    @Query("""
        SELECT r.id, r.studySessionId, r.taskId, r.scheduledDate, r.reviewNumber,
               r.isCompleted, r.completedAt, r.createdAt,
               t.name as taskName, g.name as groupName
        FROM review_tasks r
        LEFT JOIN tasks t ON r.taskId = t.id
        LEFT JOIN subject_groups g ON t.groupId = g.id
        WHERE r.scheduledDate = :date AND r.isCompleted = 0
        ORDER BY r.reviewNumber ASC
    """)
    fun getPendingTasksForDateWithDetails(date: LocalDate): Flow<List<ReviewTaskWithDetails>>
    ...
    @Query("""
        ...
        ORDER BY r.isCompleted ASC, r.reviewNumber ASC
    """)
    fun getAllTasksForDateWithDetails(date: LocalDate): Flow<List<ReviewTaskWithDetails>>
    ...
    @Query("""
        ...
        LEFT JOIN subject_groups g ON t.groupId = g.id
        WHERE r.scheduledDate <= :date AND r.isCompleted = 0
        ORDER BY r.scheduledDate ASC, r.reviewNumber ASC
    """)
    fun getOverdueAndTodayTasksWithDetails(date: LocalDate): Flow<List<ReviewTaskWithDetails>>
    ...
    /**
     * Get all review tasks with details (for settings management)
     */
    @Query("""
        SELECT r.id, r.studySessionId, r.taskId, r.scheduledDate, r.reviewNumber,
               r.isCompleted, r.completedAt, r.createdAt,
    ...
    """)
    fun getAllWithDetails(): Flow<List<ReviewTaskWithDetails>>

    /**
     * Get total count of review tasks
     */
    @Query("SELECT COUNT(*) FROM review_tasks")
    suspend fun getTotalCount(): Int

    /**
     * Get count of incomplete review tasks
     */
