package com.iterio.app.data.cloud
...
class GoogleDriveManager @Inject constructor(
    ...
    companion object {
        private const val APP_NAME = "Iterio"
        private const val BACKUP_FILE_NAME = "iterio_cloud_backup.json"
    ...
    suspend fun uploadBackup(jsonContent: String): Result<CloudBackupInfo> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            // nullチェック: execute()がnullを返す場合がある
            if (uploadedFile == null) {
                return@withContext Result.failure(
                    IllegalStateException("ファイルのアップロードに失敗しました")
                )
            }
            ...
            if (fileInfo == null) {
                return@withContext Result.failure(
                    IllegalStateException("アップロードしたファイル情報の取得に失敗しました")
                )
            }
