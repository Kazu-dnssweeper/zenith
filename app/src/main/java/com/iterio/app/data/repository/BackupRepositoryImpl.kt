...
import com.iterio.app.domain.model.*
...
class BackupRepositoryImpl @Inject constructor(
    ...
    private fun clearAllData() {
        ...
        try {
            db.execSQL("DELETE FROM review_tasks")
            db.execSQL("DELETE FROM study_sessions")
            db.execSQL("DELETE FROM tasks")
            db.execSQL("DELETE FROM subject_groups")
            db.execSQL("DELETE FROM settings")
            db.execSQL("DELETE FROM daily_stats")
        } catch (e: Exception) {
            throw IllegalStateException("データクリアに失敗しました: ${e.message}", e)
        }
    }
