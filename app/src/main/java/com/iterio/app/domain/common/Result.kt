 ...
 * Usage:
 * ```
 * fun findTask(id: Long): Result<Task, DomainError> {
 *     val task = taskDao.findById(id)
 *     return if (task != null) {
 *         Result.Success(task)
 *     } else {
 *         Result.Failure(DomainError.NotFoundError("Task not found: $id"))
 *     }
 * }
 *
 * // 使用例
 * findTask(123)
 *     .map { it.name }
 *     .onSuccess { println("Task name: $it") }
...
sealed class Result<out T, out E> {
    ...
    companion object {
        ...
        inline fun <T> catching(
            errorTransform: (Throwable) -> DomainError = { DomainError.UnknownError(it.message ?: "Unknown error", it) },
            block: () -> T
        ...
        suspend inline fun <T> catchingSuspend(
            crossinline errorTransform: (Throwable) -> DomainError = { DomainError.UnknownError(it.message ?: "Unknown error", it) },
